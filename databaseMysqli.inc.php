		...
		class DatabaseMysqli
		{
			...
			protected function select($query, $data, $format)
			{
				...
				if ( $data && count($data) && $format )
				{
					...
					list( $fields, $placeholders, $values ) = $this->prep_query($data, 'select');
			...
			protected function delete($table, $prop, $value)
			{
				if ( gettype($value) !== 's' && gettype($value) !== 'd' )
				{
					return false;
				}
				...
				$stmt = $mysqli->prepare("DELETE FROM {$table} WHERE {$prop} = ?");
				$stmt->bind_param(gettype($value) === 'string' ? 's' : 'd', $value);
				$stmt->execute();
